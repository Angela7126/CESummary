...
# ------------ some dict object ---------------
idname = {'mymodel2': '01', 'tfidf2': '02', 'graphb2': '03', 'graphw2': '04', 'context2': '05', 'mysecmodel2': '06',
           'myseccontextmodel2': '07', 'hybrid2': '08', 'sectitle2': '09'}
...
def evaluate_all_kg(fp, papers_path, pk_path, modeltype='mymodel2', inc_abscon=True, useabstr=1, maxtxt=-1, fname_topk_dict={}, bias=0.8):
    print "model type =", modeltype
    # systemdir should be managed accoding to the inc_abscon parameter
    if not inc_abscon:
        system_dir = os.path.join(fp, 'systemBias_'+str(bias)+'_1')
        system_mandir = os.path.join(fp, 'systemBias_'+str(bias)+'_man1')
    else:
        system_dir = os.path.join(fp, 'systemBias_'+str(bias)+'_2')
        system_mandir = os.path.join(fp, 'systemBias_'+str(bias)+'_man2')
    ...
    for i, file_name in enumerate(file_list):
        ...
        if modeltype == 'mymodel2':
            old_model = MyModel2(pickle_path, opt='nonce')
            sysce_model = MyModel2(pickle_path, opt='sysce', bias=bias)
            if len(old_model.text.man_CE_list) > 0:
                mance_model = MyModel2(pickle_path, opt='mance', bias=bias)
            else:
                mance_model = None
        if modeltype == 'tfidf2':
            old_model = TfIdf2(pickle_path, opt='nonce')
            sysce_model = TfIdf2(pickle_path, opt='sysce', bias=bias)
            if len(old_model.text.man_CE_list) > 0:
                mance_model = TfIdf2(pickle_path, opt='mance', bias=bias)
            else:
                mance_model = None
        if modeltype == 'graphb2':
            old_model = GraphBased2(pickle_path, opt='nonce')
            sysce_model = GraphBased2(pickle_path, opt='sysce', bias=bias)
            if len(old_model.text.man_CE_list) > 0:
                mance_model = GraphBased2(pickle_path, opt='mance', bias=bias)
            else:
                mance_model = None
        if modeltype == 'graphw2':
            old_model = WordGraph2(pickle_path, opt='nonce')
            sysce_model = WordGraph2(pickle_path, opt='sysce', bias=bias)
            if len(old_model.text.man_CE_list) > 0:
                mance_model = WordGraph2(pickle_path, opt='mance', bias=bias)
            else:
                mance_model = None
        if modeltype == 'context2':
            old_model = conTextModel2(pickle_path, opt='nonce')
            sysce_model = conTextModel2(pickle_path, opt='sysce', bias=bias)
            if len(old_model.text.man_CE_list) > 0:
                mance_model = conTextModel2(pickle_path, opt='mance', bias=bias)
            else:
                mance_model = None
        if modeltype == 'mysecmodel2':
            old_model = MySecModel2(pickle_path, opt='nonce')
            sysce_model = MySecModel2(pickle_path, opt='sysce', bias=bias)
            if len(old_model.text.man_CE_list) > 0:
                mance_model = MySecModel2(pickle_path, opt='mance', bias=bias)
            else:
                mance_model = None
        if modeltype == 'myseccontextmodel2':
            old_model = SecConTextModel2(pickle_path, opt='nonce')
            sysce_model = SecConTextModel2(pickle_path, opt='sysce', bias=bias)
            if len(old_model.text.man_CE_list) > 0:
                mance_model = SecConTextModel2(pickle_path, opt='mance', bias=bias)
            else:
                mance_model = None
        if modeltype == 'hybrid2':
            old_model = HybridGraph2(pickle_path, opt='nonce')
            sysce_model = HybridGraph2(pickle_path, opt='sysce', bias=bias)
            if len(old_model.text.man_CE_list) > 0:
                mance_model = HybridGraph2(pickle_path, opt='mance', bias=bias)
            else:
                mance_model = None
        if modeltype == 'sectitle2':  # MySecTitleNetwork.py
            old_model = MySecTitleModel2(pickle_path, opt='nonce')
            sysce_model = MySecTitleModel2(pickle_path, opt='sysce', bias=bias)
            if len(old_model.text.man_CE_list) > 0:
                mance_model = MySecTitleModel2(pickle_path, opt='mance', bias=bias)
            else:
                mance_model = None
...
def evaluate_all_acl(fp, papers_path, pk_path, modeltype='mymodel2', inc_abscon=True, useabstr=1, maxtxt=-1, topk=10, bias=0.8):
    ...
    # systemdir should be managed accoding to the inc_abscon parameter
    if not inc_abscon:
        system_dir = os.path.join(fp, 'systemBias_' + str(bias) + '_html1')
    else:
        system_dir = os.path.join(fp, 'systemBias_' + str(bias) + '_html2')
    if not os.path.exists(system_dir):
        os.makedirs(system_dir)
...
def evaluate_all_duc(fp, papers_path, pk_path, modeltype='mymodel2', inc_abscon=True, useabstr=1, maxtxt=-1, topk=10, bias=0.8):
    # -- if modeltype is not in our model list, not execute the following process --
    if modeltype not in idname.keys():
        return
...
def RankOnKG_GetModSysFiles(path, papers_path, pk_path, maxwords = 0, topksents=10):
    ...
    for bias in range(10, 21):
        ...
        for inc_abscon in [True, False]:
            evaluate_all_kg(path, papers_path, pk_path, 'mymodel2', inc_abscon, useabscon, maxwords,  fname_topk_dict, bias=bias)
            evaluate_all_kg(path, papers_path, pk_path, 'tfidf2', inc_abscon, useabscon, maxwords,  fname_topk_dict, bias=bias)
            evaluate_all_kg(path, papers_path, pk_path, 'graphb2', inc_abscon, useabscon, maxwords,  fname_topk_dict, bias=bias)
            evaluate_all_kg(path, papers_path, pk_path, 'graphw2', inc_abscon, useabscon, maxwords,  fname_topk_dict, bias=bias)
            evaluate_all_kg(path, papers_path, pk_path, 'context2', inc_abscon, useabscon, maxwords,  fname_topk_dict, bias=bias)
            evaluate_all_kg(path, papers_path, pk_path, 'mysecmodel2', inc_abscon, useabscon, maxwords,  fname_topk_dict, bias=bias)
            evaluate_all_kg(path, papers_path, pk_path, 'myseccontextmodel2', inc_abscon, useabscon, maxwords,  fname_topk_dict, bias=bias)
            evaluate_all_kg(path, papers_path, pk_path, 'hybrid2', inc_abscon, useabscon, maxwords,  fname_topk_dict, bias=bias)
            evaluate_all_kg(path, papers_path, pk_path, 'sectitle2', inc_abscon, useabscon, maxwords,  fname_topk_dict, bias=bias)
    for bias in range(10, 21):
        bias /= 20.0
        model_path = os.path.join(path, 'model_html')
        system_dir = ['systemBias_'+str(bias)+'_man1', 'systemBias_'+str(bias)+'_man2', 'systemBias_'+str(bias)+'_1', 'systemBias_'+str(bias)+'_2']
        system_idlst = [['01', '21', '02', '22', '03', '23', '04', '24', '05', '25', '06', '26', '07', '27', '08', '28', '09', '29'],
...
def RankOnACL_GetModSysFiles(path, papers_path, pk_path, maxwords=0, topksents=10):
    ...
    for bias in range(10, 21):
        ...
        print "current bias = ", bias
        for inc_abscon in [True, False]:
            evaluate_all_acl(path, papers_path, pk_path, 'mymodel2', inc_abscon, useabscon, maxwords,  topk=topksents, bias=bias)
            evaluate_all_acl(path, papers_path, pk_path, 'tfidf2', inc_abscon, useabscon, maxwords,  topk=topksents, bias=bias)
            evaluate_all_acl(path, papers_path, pk_path, 'graphb2', inc_abscon, useabscon, maxwords,  topk=topksents, bias=bias)
            evaluate_all_acl(path, papers_path, pk_path, 'graphw2', inc_abscon, useabscon, maxwords,  topk=topksents, bias=bias)
            evaluate_all_acl(path, papers_path, pk_path, 'context2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
            evaluate_all_acl(path, papers_path, pk_path, 'mysecmodel2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
            evaluate_all_acl(path, papers_path, pk_path, 'myseccontextmodel2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
            evaluate_all_acl(path, papers_path, pk_path, 'hybrid2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
            evaluate_all_acl(path, papers_path, pk_path, 'sectitle2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
    for bias in range(10, 21):
        bias /= 20.0
        model_path = os.path.join(acl_dir, 'model_html')
        system_dir = ['systemBias_' + str(bias) + '_html1', 'systemBias_' + str(bias) + '_html2']
        system_idlst = [['01', '31', '02', '32', '03', '33', '04', '34', '05', '35', '06', '36', '07', '37', '08', '38', '09', '39']]
...
def RankOnDUC_GetModSysFiles(path, papers_path, pk_path, maxwords=0, topksents=10):
    ...
    for bias in range(1, 21):
        ...
        print "current bias = ", bias
        for inc_abscon in [True, False]:
            evaluate_all_duc(path, papers_path, pk_path, 'mymodel2', inc_abscon, useabscon, maxwords,  topk=topksents, bias=bias)
            evaluate_all_duc(path, papers_path, pk_path, 'tfidf2', inc_abscon, useabscon, maxwords,  topk=topksents, bias=bias)
            evaluate_all_duc(path, papers_path, pk_path, 'graphb2', inc_abscon, useabscon, maxwords,  topk=topksents, bias=bias)
            evaluate_all_duc(path, papers_path, pk_path, 'graphw2', inc_abscon, useabscon, maxwords,  topk=topksents, bias=bias)
            evaluate_all_duc(path, papers_path, pk_path, 'context2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
            evaluate_all_duc(path, papers_path, pk_path, 'mysecmodel2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
            evaluate_all_duc(path, papers_path, pk_path, 'myseccontextmodel2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
            evaluate_all_duc(path, papers_path, pk_path, 'hybrid2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
            evaluate_all_duc(path, papers_path, pk_path, 'sectitle2', inc_abscon, useabscon, maxwords, topk=topksents, bias=bias)
    for bias in range(1, 21):
        bias /= 20.0
