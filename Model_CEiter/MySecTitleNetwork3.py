...
# ------------------------------------- Classes ------------------------------------- #
#######################################################################################
class MySecTitleModel3:
    # choice: 1 means with stopwords; 2 means without stopwords
    def __init__(self, pickle_path, choice=2, iteration_times=40, opt='nonce', bias=0.5):
        self.w_s = None
        self.s_p = None
        self.p_c = None
        self.c_c = None
        ...
        # self.text.word_count = self.Buildwordcount()  # add a new attribute named as "word_count" to sxpText object
        # self.NormalizeWS()
...
def TestPickle():
    ...
    model = MySecTitleModel3(pk, opt='mance')
    topksent = 10
    tops = model.OutPutTopKSentWeight(topksent, 0, 500)
