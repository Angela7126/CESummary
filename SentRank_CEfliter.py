...
idname = {'mymodel': '01', 'tfidf': '02', 'graphb': '03', 'graphw': '04', 'context1': '05', 'mysecmodel': '06',
           'myseccontextmodel': '07', 'hybrid': '08', 'sectitle': '09'}
...
#######################################################################################
def OutPutTopKmanCESent(model, topk, useabstr = 1, maxwords = -1):
    ...
    for sentence in model.ranked_sentences:
        ...
        if wordlen >= abstractlen and maxwords == -1:  # maxwords == 1 means use abstractlen str as upper found
            break
        if len(sentence.sentence_text) <= 1:  # ignore sentence that contains less than 2 str
            continue
...
def OutPutTopKsysCESent(model, topk, useabstr = 1, maxwords = -1):
    i = 0  # i stores how many sentences have been append into sent_txt_set
    wordlen = 0  # wordlen stores how many str has been append into sent_txt_set
    # useabstr == 1, use number of str in abstract as the maxwords
    if useabstr == 1:
...
def evaluate_all_kg(fp, papers_path, pk_path, modeltype='mymodel', inc_abscon=True, useabstr=1, maxtxt=-1, fname_topk_dict={}):
    ...
    if not inc_abscon:
        system_dir = os.path.join(fp, 'systemFliter_1')
        system_mandir = os.path.join(fp, 'systemFliter_man1')
    else:
        system_dir = os.path.join(fp, 'systemFliter_2')
        system_mandir = os.path.join(fp, 'systemFliter_man2')
    ...
    for i, file_name in enumerate(file_list):
        ...
        mance_topk = topk if topk <= len(model.text.mance_sent_id_dict) else len(model.text.mance_sent_id_dict)
        if mance_topk > 0:
            topk_mance_sent_path = os.path.join(system_mandir, file_name+'.html.'+str(20 + int(idstr)))
            topk_mance_sent = OutPutTopKmanCESent(model, mance_topk, useabstr, maxtxt)
            mance_st = ProduceSystem(topk_mance_sent, file_name, 1)
            WriteStrFile(topk_mance_sent_path, mance_st, 'utf-8')
...
def evaluate_all_acl(fp, papers_path, pk_path, modeltype='mymodel', inc_abscon=True, useabstr=1, maxtxt=-1, topk=10):
    ...
    if not inc_abscon:
        ...
        rankedsentdir = os.path.join(fp, 'RankedSentFliter_1')
    else:
        system_dir = os.path.join(fp, 'systemFliter_html2')
        rankedsentdir = os.path.join(fp, 'RankedSentFliter_2')
    if not os.path.exists(system_dir):
        os.makedirs(system_dir)
    ...
    for i, file_name in enumerate(file_list):
        ...
        StoreSxptext(model.ranked_sentences, ranked_sent_fp)
        # -- save original model ranked topk sentences to text --
        topksent_path = os.path.join(system_dir, file_name+'.html.'+idstr)
        tops = model.OutPutTopKSent(topk, useabstr, maxtxt)
        st = ProduceSystem(tops, file_name, 1)
...
def RankOnKG_GetModSysFiles(path, papers_path, pk_path, maxwords = 0, topksents=10):
    ...
    # system_dir = ['systemFliter_man_sys_1', 'systemFliter_man_sys_2']
    # system_idlst = [['21', '31', '22', '32', '23', '33', '24', '34', '25', '35', '26', '36', '27', '37', '28', '38', '29', '39']]
...
def RankOnACL_GetModSysFiles(path,  papers_path, pk_path, maxwords=-1, topksents=10):
    ...
    system_dir = ['systemFliter_html1', 'systemFliter_html2']
